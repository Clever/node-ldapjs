...
function Client (options) {
  ...
  // Note: tls.Socket is a subclass of net.Socket
  // onSocketDefined allows continuing setup on the socket without waiting for the socket to be connected, and
  // also allows setting up timeouts which may interact with the connection process itself, and setting up an on('error') which
  // can handle connection errors
  // createConnection should call onSocketDefined *before* adding the onConnect handler
  // onSocketDefined allows the LDAP client to save a reference to the socket,
  // which is needed for onConnect to work.
  // onError allows any other errors, such as errors setting up the connection itself,
  // to propagate to the ldap client.
...
Client.prototype.connect = function connect () {
  ...
  function connectSocket (cb) {
    ...
    function onSocketDefined (definedSocket) {
      socket = definedSocket
    ...
    if (self.createConnection) {
      self.createConnection(port, host, onConnect, onSocketDefined, onResult)
    } else if (server && server.secure) {
      socket = tls.connect(port, host, self.tlsOptions)
      socket.once('secureConnect', onConnect)
      onSocketDefined(socket)
    } else {
